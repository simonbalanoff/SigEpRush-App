...
struct AddPNMWizard: View {
    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                content
                HStack {
                    if vm.step > 0 { Button("Back") { vm.step -= 1 } }
                    Spacer()
                    if vm.step < 2 {
                        Button("Next") {
            ...
            .sheet(isPresented: $showLibrary) { ImagePicker(sourceType: .photoLibrary) { img in vm.image = img } }
            .sheet(isPresented: $showCamera) { ImagePicker(sourceType: .camera) { img in vm.image = img } }
            .confirmationDialog("Select Photo", isPresented: $showPickerOptions) {
                Button("Choose from Library") { showLibrary = true }
                Button("Take Photo") { showCamera = true }
                if vm.image != nil { Button("Remove Photo", role: .destructive) { vm.image = nil } }
    ...
    @ViewBuilder var content: some View {
        switch vm.step { case 0: infoView; case 1: photoView; default: reviewView }
    }
        ...
        Form {
            ...
            Section {
                ...
                HStack {
                    ...
                    Stepper("", value: $vm.gradYearDelta, in: -6...6).labelsHidden()
                }
            }
